...
def nt_pinn(model, input_data, t, output_data, nu, scheme):
    ...
    # Select x for only between -1 and 1, select the index
    #index_y = np.where((y > -1) & (y < 1))
    #index_y = np.array(index_y)
    #index_y = index_y[:,::XX.shape[1]]; index_y = index_y[0]
...
def hmc_sampling(model, X, Y, num_samples, step_size, num_leapfrog_steps):
    ...
    with open('B1NNN-NS-' + str(num_leapfrog_steps) + '.pkl', 'wb') as f:
